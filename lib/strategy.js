...
function Strategy(options, verify) {
  options = options || {};
  this._baseURL = options.baseURL || 'http://127.0.0.1';
  options.authorizationURL = options.authorizationURL || url.resolve(this._baseURL, 'oauth2/auth');
  options.tokenURL = options.tokenURL || url.resolve(this._baseURL, 'oauth2/token');
  options.scope = options.scope || 'read_user';
  options.scopeSeparator = options.scopeSeparator || ',';
  options.state = new Array(3).fill(0).map(k => Math.random().toString(36).substring(7)).join('');

  OAuth2Strategy.call(this, options, verify);
  this.name = 'hydra';
  this._profileURL = options.profileURL || url.resolve(this._baseURL, 'userinfo');
